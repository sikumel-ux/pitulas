...
function renderItems(data) {
    ...
    data.forEach(item => {
        ...
        if(item.kategori === 'Video') {
            ...
            // Logic Gambar: Hanya muncul jika ada data urlGambar
            const imgTag = item.urlGambar 
                ? `<img src="${item.urlGambar}" class="w-full h-auto max-h-64 object-cover rounded-2xl mb-4 shadow-sm" alt="Post Image">` 
                : '';
            ...
            card.innerHTML = `
                ...
                ${imgTag}
                <h4 class="font-bold text-lg mb-2 text-gray-800">${item.judul}</h4>
                <p class="content-text text-sm text-gray-600 leading-relaxed">${item.isi.replace(/\n/g, '<br>')}</p>
...
window.filterCategory = (cat, el) => {
    // Update active state di navigasi dan kategori
